...
ifneq (python_version_minor,7)
	...
	@if hash brew 2>/dev/null; then brew install -q python@3.7 libpq; python3.7 -m pip install virtualenv; fi;
	@if hash apt-get 2>/dev/null; then sudo apt-get update && sudo apt-get install software-properties-common; fi;
	@if hash add-apt-repository 2>/dev/null; then sudo add-apt-repository ppa:deadsnakes/ppa; sudo apt-get install python3.7; fi;
endif
ifeq (python_version_minor,7)
	...
	#@$(shell command -v python3.7) -m pip install -U -r requirements.txt
	@$(shell command -v python3.7) -m pip install -U -r requirements.lock
endif
endif
