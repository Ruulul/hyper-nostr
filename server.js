...
exec(`gnostr --sec ${sdk.publicKey.toString('hex')} --tag "gnostr" "repo" --tag "weeble" "$(gnostr-weeble)" --tag "wobble" "$(gnostr-wobble)" --tag "blockheight" "$(gnostr-blockheight)" --envelope --content "${content}" `, (error, stdout, stderr) => {
  if (error) {
    console.log(`gnostr-lfs error: ${error.message}`)
    return
  }
