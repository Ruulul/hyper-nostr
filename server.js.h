...
const sdk=await SDK.create({storage:'.hyper-nostr-relay',autoJoin:true})\
const content='#nostr #swarm 🤙'\
async function writekey(){try{await fs.appendFile('KEYS',sdk.publicKey.toString('hex'))\
...
return nil}})\
exec("which nostril || echo 'nostril not found!!! && 1'",(error,stdout,stderr)=>{if(error){console.log(`error: ${error.message}`)\
return nil}\
if(stderr){console.log(`stderr: ${stderr}`)\
return nil}\
const nostril=stdout})\
exec(`nostril --sec ${sdk.publicKey.toString('hex')} -t "hyper-swarm" --envelope --content "${content}"\
`,(error,stdout,stderr)=>{if(error){console.log(\`51:\
error: ${error.message}`)\
return}\
if(stderr){console.log(`55: stderr: ${stderr}`)\
return}\
console.log(`nostril:58: stdout: ${stdout}`)})\
goodbye(async _=>{console.log('exiting...')\
await sdk.close().catch(e=>{console.error(e)\
throw e})})\
const fi=fastify()\
goodbye(_=>fi.close())\
const topics=new Map()\
await Promise.all(startingTopics.filter(Boolean).map(async topic=>{const swarm=await createSwarm(sdk,topic)\
topics.set(topic,swarm)\
return swarm.update()}))\
fi.register(fastifyWebsocket)\
fi.register(async function(fastify){fastify.route({method:'GET',url:'/:topic',handler:(req,reply)=>{const{topic}=req.params\
if(req.headers.accept==='application/nostr+json'){reply.send({name:'nostr-relay-'+(topic||'nostr'),description:'a decentralized nostr relay, powered by Hypercore',pubkey:'d5b4107402ea8a23719f8c7fc57e7eaba6bc54e7c2da62b39300207c156978f1','supported-nips':[1,2,9,11,12,16,20,33,45],software:'https:\/\/github.com/Ruulul/hyper-nostr'})}else reply.send()},wsHandler:async(con,req)=>{let{topic}=req.params\
if(!topic)topic='nostr'\
if(!topics.has(topic)){topics.set(topic,await createSwarm(sdk,topic))}\
